  ...
  frameWatchdog = setTimeout(() => {
    if (!hasRenderedFrame && isConnected) {
      ...
      hudStatusText.textContent =
        'Connected to the server, but no video frames are arriving — make sure the phone app is streaming (v1.7.1+)';
...
if (btnVcam) {
  ...
  btnVcam.addEventListener('click', async () => {
    ...
    if (res && res.success) {
      showToast('OpenCam Virtual Camera registered! Fully quit WhatsApp (system tray) and restart Discord to see it.', 7000);
    } else {
